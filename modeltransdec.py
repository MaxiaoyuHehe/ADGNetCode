...
class IQARegressionDec(nn.Module):
    ...
    def forward(self, enc_inputs, x, dec_inputs, y):
        # batch x (320*6) x 29 x 29 -> batch x 256 x 29 x 29
        enc_inputs_embed = self.conv_enc(x)
        dec_inputs_embed = self.conv_enc(y)
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_inputs_embed, dec_inputs, dec_inputs_embed):
        ...
        dec_outputs, dec_self_attn_probs, dec_enc_attn_probs = self.decoder(dec_inputs, dec_inputs_embed, enc_inputs,
                                                                            enc_outputs)
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, inputs_embed):
        ...
        positions = torch.arange(inputs.size(1), device=inputs.device, dtype=torch.int64).expand(inputs.size(0),
                                                                                                 inputs.size(
                                                                                                     1)).contiguous() + 1
